...
# 1) Make contact identity confidence explicit.
...
# 2) Guarantee backend contact lookup ends quickly.
...
# 3) Replace progressive contact queue block. Process ALL visible playlists, never leave a spinner,
#    and publish contactable results into the existing Playlist contattabili section.
...
# 4) Reset the global contactable section immediately when a new Artist Radar scan starts.
