const VERSION = 'RADAR v0.4.7.27 Cloud';
...
async function resolvePromotedTrack(){
 ...
 try{const r=await fetch('/api/spotify-track?url='+encodeURIComponent(clean));const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||'Brano Spotify non trovato');promotedTrack=d;box.innerHTML='';if(d.image){const im=document.createElement('img');im.src=d.image;im.alt='';im.style.cssText='width:54px;height:54px;border-radius:9px;object-fit:cover;flex:0 0 54px';box.appendChild(im)}const tx=document.createElement('div');const t=document.createElement('strong');t.textContent=d.track||'Brano Spotify';const a=document.createElement('div');a.textContent=d.artist||'';a.style.cssText='font-size:12px;font-weight:800;letter-spacing:.04em;opacity:.82;margin-bottom:3px';tx.append(a,t);box.appendChild(tx);const g=$('#genre');if(g&&d.genres&&d.genres.length){const detected=d.genres[0];if(!g.value.trim())g.value=detected;let note=$('#genreDetected');if(!note){note=document.createElement('div');note.id='genreDetected';note.style.cssText='font-size:11px;color:var(--green);margin-top:6px';g.parentNode.appendChild(note)}note.textContent='Rilevato automaticamente: '+d.genres.join(' · ')}}catch(e){promotedTrack=null;box.textContent=e.message||'Errore Spotify'}
}
