...
<div class="resultsHead"><h2>Playlist rilevate</h2><span class="count" id="count">0 risultati</span></div>
<div id="results" class="cards"><div class="empty">Imposta il genere e avvia RADAR. I punteggi sono basati su segnali web osservabili, non su metriche Spotify derivate.</div></div>
</main>
<script>
...
function render(items){const box=$('#results');$('#count').textContent=items.length+' risultati';if(!items.length){box.innerHTML='<div class="empty">Nessuna playlist utile trovata. Prova un genere più ampio o Analisi Completa.</div>';return}box.innerHTML=items.map((r,i)=>'<article class="card"><div class="top"><div><div class="title">'+esc(r.name)+'</div><div class="source">'+esc(r.snippet||r.sourceTitle||'Segnale web pubblico')+'</div></div><span class="badge '+badgeClass(r.badge)+'">'+esc(r.badge)+'</span></div><div class="metrics">'+metric('RADAR Score',r.score)+metric('Activity',r.activity)+metric('Match',r.match)+metric('Confidence',r.confidence)+'</div><div class="why"><b>PERCHÉ QUESTA PLAYLIST?</b><br>'+esc(r.why)+'</div><div class="cardActions"><a class="linkbtn" target="_blank" rel="noopener" href="'+esc(r.spotifyUrl)+'">Apri su Spotify</a><button class="curatorBtn" data-index="'+i+'" data-playlist-name="'+esc(r.name)+'" data-spotify-url="'+esc(r.spotifyUrl)+'">Trova curatore / contatti</button></div><div class="curatorBox" id="curator-'+i+'"></div></article>').join('')}
async function discover(){const b=$('#discover');b.disabled=true;$('#status').textContent='Scansione web in corso…';try{const res=await fetch('/api/discover',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({genre:$('#genre').value.trim(),artists:$('#artists').value.trim(),mode:$('#mode').value,strategy:$('#strategy').value})});const data=await res.json();if(!res.ok)throw new Error(data.error||'Errore ricerca');render(data.results||[]);$('#status').textContent=(data.braveConfigured?'Brave attivo':'Demo')+' · '+(data.results||[]).length+' playlist'}catch(e){$('#status').innerHTML='<span class="error">'+esc(e.message)+'</span>'}finally{b.disabled=false}}
...
async function discover(input,env){const genre=String(input.genre||'').trim();if(!genre)return{results:[],braveConfigured:!!env.BRAVE_API_KEY};if(!env.BRAVE_API_KEY){return{braveConfigured:false,results:[{name:genre+' — Demo Playlist',spotifyUrl:'https://open.spotify.com/',snippet:'BRAVE_API_KEY non configurata nel Worker.',sourceTitle:'Demo',...scoreResult({title:genre+' playlist updated curator submission',description:'demo'},input)}]}}
const queries=buildQueries(input);const batches=await Promise.all(queries.map(q=>braveSearch(q,env,10).catch(()=>[])));const map=new Map();for(const r of batches.flat()){const spotifyUrl=cleanPlaylistUrl(r.url)||cleanPlaylistUrl(r.description)||cleanPlaylistUrl(r.title);if(!spotifyUrl)continue;if(map.has(spotifyUrl))continue;const s=scoreResult(r,input);map.set(spotifyUrl,{name:cleanTitle(r.title),spotifyUrl,snippet:r.description||'',sourceTitle:r.title||'',...s})}return{braveConfigured:true,results:[...map.values()].sort((a,b)=>b.score-a.score).slice(0,input.mode==='complete'?50:30)}}
