const VERSION = 'RADAR v0.4.7.55 Cloud';
...
async function scanArtistRadar(){const input=document.getElementById('artistRadarInput'),btn=document.getElementById('artistRadarScan'),st=document.getElementById('artistRadarStatus'),sum=document.getElementById('artistRadarSummary'),box=document.getElementById('artistRadarResults');if(!input||!btn||!st||!sum||!box)return;const artist=input.value.trim();if(!artist){st.textContent='Inserisci il nome artista o il link Spotify.';return}btn.disabled=true;st.textContent='Analizzo catalogo, placement e verifiche pubbliche…';sum.style.display='none';box.innerHTML='';try{const r=await fetch('/api/artist-radar',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({artist:artist})});const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||'Errore Artist Radar');const tracks=Array.isArray(d.tracks)?d.tracks:[],rows=Array.isArray(d.playlists)?d.playlists:[],vs=d.verificationSummary||{};sum.style.display='block';const cs=d.contactSummary||{};sum.textContent=(d.artist||artist)+' · '+tracks.length+' tracce · '+rows.length+' playlist · '+(vs.confirmed||0)+' confermate · '+(vs.probable||0)+' probabili · '+(cs.contactable||0)+' contattabili · '+String(d.build||'RADAR')+(d.catalogSource?' · catalogo '+d.catalogSource:'');st.textContent=rows.length?'Verifica completata: '+(vs.confirmed||0)+' confermate, '+(vs.probable||0)+' probabili, '+(vs.web||0)+' solo evidenza web.':'Nessuna playlist pubblicamente rilevabile.';for(const x of rows){const card=document.createElement('div');card.className='result';const h=document.createElement('h3');h.textContent=x.name||'Spotify playlist';const verify=document.createElement('div');verify.className='muted';verify.style.cssText='margin:6px 0;font-weight:900';verify.textContent=(x.verification==='CONFIRMED'?'🟢 ':x.verification==='PROBABLE'?'🟡 ':'⚪ ')+(x.verificationLabel||'SOLO EVIDENZA WEB');const meta=document.createElement('div');meta.className='muted';meta.textContent=(x.owner?x.owner+' · ':'')+(x.trackCount||0)+' tracce dell’artista rilevate';const names=document.createElement('div');names.className='muted';names.style.marginTop='6px';names.textContent=(x.tracks||[]).slice(0,8).join(' · ');card.append(h,verify,meta,names);const c=x.contact||{};if(c.contactable){const cp=document.createElement('div');cp.className='muted';cp.style.cssText='margin-top:9px;padding:9px;border:1px solid #263052;border-radius:11px';const bits=[];if(c.email)bits.push('✉ '+c.email);if(c.instagram)bits.push('Instagram');if(c.submission)bits.push('Submission');if(c.website)bits.push('Sito');cp.textContent='CONTATTABILE '+(c.score||0)+'/100 · '+bits.join(' · ');card.appendChild(cp)}card.appendChild(document.createTextNode(''));if(x.spotifyUrl){const a=document.createElement('a');a.href=x.spotifyUrl;a.target='_blank';a.rel='noopener';a.className='btn secondary';a.style.cssText='display:inline-block;margin-top:10px;text-decoration:none';a.textContent='Apri Spotify';card.appendChild(a)}box.appendChild(card)}}catch(e){st.textContent=e.message||'Errore Artist Radar'}finally{btn.disabled=false}}
