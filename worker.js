const VERSION = 'RADAR v0.4.7.64 Cloud';
...
function setRadarView(view){const track=view==='track';const shell=$('#trackRadarShell');const grid=document.querySelector('.panel > .grid');const actions=document.querySelector('.panel > .actions');if(shell)shell.style.display=track?'block':'none';if(grid)grid.style.display=track?'none':'grid';if(actions)actions.style.display=track?'none':'flex';const d=$('#discoveryRadarTab'),t=$('#trackRadarTab');if(d){d.style.borderColor=track?'':'var(--green)';d.style.color=track?'':'var(--green)'}if(t){t.style.borderColor=track?'var(--green)':'';t.style.color=track?'var(--green)':''}}
document.addEventListener('click',function(e){if(e.target&&e.target.id==='discoveryRadarTab')setRadarView('discovery');if(e.target&&e.target.id==='trackRadarTab')setRadarView('track')});
let trackRadarMeta=null;
async function resolveTrackRadar(){const el=$('#trackRadarUrl'),preview=$('#trackRadarPreview'),status=$('#trackRadarStatus'),btn=$('#trackRadarScan');if(!el||!preview||!status||!btn)return;const raw=el.value.trim();let u;try{u=new URL(raw)}catch(e){trackRadarMeta=null;preview.style.display='none';btn.disabled=true;status.textContent='Incolla un link Spotify valido.';return}const parts=u.pathname.split('/').filter(Boolean);if(u.hostname!=='open.spotify.com'||parts[0]!=='track'||!parts[1]){trackRadarMeta=null;preview.style.display='none';btn.disabled=true;status.textContent='Serve un link Spotify di un brano.';return}const clean='https://open.spotify.com/track/'+parts[1];status.textContent='Riconosco il brano…';btn.disabled=true;try{const r=await fetch('/api/spotify-track?url='+encodeURIComponent(clean));const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||'Brano non trovato');trackRadarMeta=d;preview.innerHTML='';preview.style.display='flex';if(d.image){const im=document.createElement('img');im.src=d.image;im.alt='';im.style.cssText='width:54px;height:54px;border-radius:9px;object-fit:cover;flex:0 0 54px';preview.appendChild(im)}const tx=document.createElement('div');const ar=document.createElement('div');ar.textContent=d.artist||'';ar.style.cssText='font-size:12px;font-weight:800;opacity:.82';const tt=document.createElement('strong');tt.textContent=d.track||'Brano Spotify';tx.append(ar,tt);preview.appendChild(tx);btn.disabled=false;status.textContent='Brano riconosciuto · pronto per il motore placement.'}catch(e){trackRadarMeta=null;preview.style.display='none';btn.disabled=true;status.textContent=e.message||'Errore Spotify'}}
setTimeout(()=>{const el=$('#trackRadarUrl');if(el){el.addEventListener('input',()=>{clearTimeout(el._tr);el._tr=setTimeout(resolveTrackRadar,220)});el.addEventListener('paste',()=>setTimeout(resolveTrackRadar,30))}},0);
...
async function artistRadarRunContactQueue(rows,slots,sum,baseText,token){
  for(let i=0;i<rows.length;i++){if(slots[i]&&rows[i]&&rows[i].verification!=='CONFIRMED'&&rows[i].verification!=='PROBABLE')slots[i].textContent='Contatti sospesi: placement non verificato.'}const jobs=rows.map((x,i)=>({x,slot:slots[i]})).filter(j=>j.slot&&(j.x.verification==='CONFIRMED'||j.x.verification==='PROBABLE')),state={next:0,done:0,found:[]};
...
function priorityOutreach(r){
  ...
  if(r.email){
    ...
    paintResults(sortedFilteredResults());
    updateOutreachBar();
    const bar=$('#outreachBar');if(bar&&selectedOutreach.has(k))bar.scrollIntoView({behavior:'smooth',block:'nearest'});
    return;
  }
  const channels=[
...
async function discover(){
  ...
  try{
    ...
    const baseRes=await fetch('/api/discover-base',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload),signal:radarAbortController.signal});
    ...
    for(let i=0;i<candidates.length;i+=chunkSize){
      ...
      const er=await fetch('/api/contact-enrich',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...payload,candidates:chunk,googleSlots:remainingGoogle}),signal:radarAbortController.signal});
...
function artistRadarCatalogMaps(cat){
...
async function artistRadarVerifyCandidate(x,cat,env){
  const direct=await artistRadarDirectPlaylistCheck(x,cat,env);
  if(direct.matches&&direct.matches.length)return {...x,verification:'CONFIRMED',verificationLabel:'VERIFICATO DIRETTAMENTE',verificationScore:100,placementConfidence:100,placementBaseScore:100,placementVerificationBoost:0,placementReason:'Traccia ORBYT trovata direttamente nella tracklist pubblica Spotify.',directVerified:true,directSource:direct.source,directMatchedTracks:direct.matches,directVisibleTracks:direct.visibleTracks,directTotalTracks:direct.totalTracks,directPartial:direct.partial,directReason:direct.reason||'',directParser:direct.parser||'',directParserPath:direct.parserPath||'',directVisibleTracks:direct.visibleTracks||0,directTotalTracks:direct.totalTracks||null,verificationDomains:['open.spotify.com'],verificationEvidence:['spotify-embed trackList']};
  if(direct.available&&direct.complete)return {...x,verification:'REJECTED',verificationLabel:'SCARTATO · NON PRESENTE',verificationScore:0,placementConfidence:0,placementBaseScore:0,placementVerificationBoost:0,placementReason:'Tracklist pubblica Spotify completa controllata: nessuna traccia del catalogo artista trovata.',directVerified:false,directSource:direct.source,directMatchedTracks:[],directVisibleTracks:direct.visibleTracks,directTotalTracks:direct.totalTracks,directPartial:false,directReason:direct.reason||'',directParser:direct.parser||'',directParserPath:direct.parserPath||'',directVisibleTracks:direct.visibleTracks||0,directTotalTracks:direct.totalTracks||null,verificationDomains:['open.spotify.com'],verificationEvidence:['spotify-embed complete no-match']};
  const tracks=[...x.tracks].slice(0,3),domains=new Set(),evidence=[],exactSpotify=new Set(),independentTracks=new Set(),nameMatchedTracks=new Set(),ownerMatchedTracks=new Set(),playlistName=String(x.name||'').trim(),owner=String(x.owner||'').trim(),playlistUrl=cleanPlaylistUrl(x.spotifyUrl),initialEvidence=Array.isArray(x.evidence)?x.evidence.filter(Boolean):[],trackCount=Number(x.trackCount||tracks.length||0);
  let baseScore=0;if(playlistUrl)baseScore+=28;if(initialEvidence.length)baseScore+=8;if(playlistName&&normalize(playlistName)!=='spotify playlist')baseScore+=5;if(owner)baseScore+=6;if(trackCount>=2)baseScore+=4;if(x.stage==='fast')baseScore+=3;else if(x.stage==='deep')baseScore+=1;baseScore=Math.min(50,baseScore);
  const jobs=[];for(const track of tracks){const base='"'+track+'" "'+cat.artist+'"';jobs.push({track,p:smartSearch(base+' "'+playlistName.replace(/"/g,'')+'"',env,8)})}
  const batches=await Promise.all(jobs.map(async j=>({track:j.track,batch:await j.p})));
  for(const j of batches){for(const r of j.batch.results||[]){const title=String(r.title||''),desc=String(r.description||''),url=String(r.url||''),blob=title+' '+desc+' '+url,text=normalize(blob);if(!text.includes(normalize(j.track))||!text.includes(normalize(cat.artist)))continue;const host=(()=>{try{return new URL(url).hostname.replace(/^www\./,'')}catch(e){return''}})(),pu=cleanPlaylistUrl(url)||cleanPlaylistUrl(blob),exactUrl=!!(pu&&playlistUrl&&pu===playlistUrl),nameMatch=artistRadarNameMatch(playlistName,title+' '+desc),ownerMatch=!!(owner&&normalize(blob).includes(normalize(owner)));if(!exactUrl&&!nameMatch)continue;if(host)domains.add(host);if(exactUrl)exactSpotify.add(j.track);if(nameMatch)nameMatchedTracks.add(j.track);if(ownerMatch)ownerMatchedTracks.add(j.track);if(host&&host!=='open.spotify.com'&&host!=='spotify.com')independentTracks.add(j.track);if(evidence.length<5)evidence.push(blob.slice(0,260))}}
  const independentDomains=[...domains].filter(d=>d&&d!=='open.spotify.com'&&d!=='spotify.com'),exactCount=exactSpotify.size,independentCount=independentTracks.size,nameCount=nameMatchedTracks.size,ownerCount=ownerMatchedTracks.size;let boost=0;if(exactCount)boost+=20;if(nameCount)boost+=8;if(independentCount)boost+=18;if(independentDomains.length>=2)boost+=5;if(ownerCount)boost+=4;const raw=Math.min(79,baseScore+boost),partial=!!direct.partial;let verification='WEB_EVIDENCE',verificationLabel=partial?'CANDIDATO · TRACKLIST PARZIALE':'SOLO EVIDENZA WEB';if(raw>=55){verification='PROBABLE';verificationLabel=partial?'PROBABILE · TRACKLIST PARZIALE':'PROBABILE'}
  return {...x,verification,verificationLabel,verificationScore:raw,placementConfidence:raw,placementBaseScore:baseScore,placementVerificationBoost:boost,placementReason:partial?'Spotify Embed disponibile ma tracklist parziale: presenza non certificabile se la traccia non appare nella porzione visibile.':'Verifica diretta Spotify non disponibile; confidenza basata solo su evidenza pubblica secondaria.',directVerified:false,directSource:direct.source||'',directMatchedTracks:[],directVisibleTracks:direct.visibleTracks||0,directTotalTracks:direct.totalTracks||null,directPartial:partial,directReason:direct.reason||'',directParser:direct.parser||'',directParserPath:direct.parserPath||'',directVisibleTracks:direct.visibleTracks||0,directTotalTracks:direct.totalTracks||null,directSource:direct.source||'',directAttempts:direct.attempts||[],directErrorMessage:direct.errorMessage||'',verificationDomains:[...domains],verificationEvidence:evidence,exactTrackEvidence:exactCount,independentTrackEvidence:independentCount,nameTrackEvidence:nameCount,ownerTrackEvidence:ownerCount};
}
...
async function artistRadarContactLookup(input,env){
  ...
  const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('Contact timeout')),9500));
...
function trackRadarAlgorithmic(name){const n=normalize(name).replace(/[^a-z0-9 ]/g,' ').replace(/\s+/g,' ').trim();return /^(release radar|discover weekly|daylist|on repeat|repeat rewind|smart shuffle|dj|radio)(?: |$)/.test(n)||/^daily mix(?: \d+)?(?: |$)/.test(n)}
async function trackRadarScan(input,env){const track=String(input?.track||'').trim(),artist=String(input?.artist||'').trim();if(!track||!artist)throw new Error('Metadati brano mancanti');const queries=[
  '"'+track+'" "'+artist+'" playlist',
  '"'+track+'" "'+artist+'" "playlist by"',
  '"'+track+'" "'+artist+'" Spotify playlist',
  '"'+track+'" '+artist+' open.spotify.com/playlist',
  'site:open.spotify.com/playlist "'+track+'" "'+artist+'"',
  'site:open.spotify.com/playlist "'+track+'" '+artist,
  '"'+track+'" '+artist+' playlist Spotify',
  '"'+track+'" "'+artist+'" curated playlist',
  '"'+track+'" "'+artist+'" tracklist playlist'
];const batches=await Promise.all(queries.map(q=>smartSearch(q,env,30)));const rawRows=batches.flatMap(x=>x.results||[]);const diagnostics={queries:queries.map((q,i)=>({query:q,provider:batches[i]?.provider||'',count:(batches[i]?.results||[]).length})),rawCount:rawRows.length,spotifyCandidates:[],rawSample:rawRows.slice(0,12).map(r=>({title:String(r.title||'').slice(0,100),url:String(r.url||'').slice(0,180),description:String(r.description||'').slice(0,180)}))};const map=new Map();for(const r of rawRows){const text=normalize((r.title||'')+' '+(r.description||'')+' '+(r.url||''));let clean=cleanPlaylistUrl(r.url||'');if(!clean){const blob=String((r.title||'')+' '+(r.description||''));const m=blob.match(/https?:\/\/open\.spotify\.com\/playlist\/[A-Za-z0-9]+/i);if(m)clean=cleanPlaylistUrl(m[0])}if(!clean)continue;const trackHit=text.includes(normalize(track)),artistHit=text.includes(normalize(artist));diagnostics.spotifyCandidates.push({url:clean,title:String(r.title||'').slice(0,120),trackHit,artistHit});if(!trackHit||!artistHit)continue;const parts=new URL(clean).pathname.split('/').filter(Boolean);if(parts[0]!=='playlist'||!parts[1])continue;const id=parts[1],evidence=String((r.title||'')+' — '+(r.description||'')).trim();const titleText=normalize(r.title||''),descText=normalize(r.description||'');const explicit=descText.includes(normalize(track))&&descText.includes(normalize(artist));const confidence=explicit?'confirmed':'probable';if(!map.has(id)||evidence.length>map.get(id).evidence.length)map.set(id,{playlistId:id,spotifyUrl:clean,name:String(r.title||'Spotify playlist').trim(),confidence,evidence:evidence.slice(0,280)});}const placements=[];for(const item of [...map.values()].slice(0,20)){try{const ident=await spotifyPlaylistIdentity(item.spotifyUrl,env);if(ident){item.name=ident.name||item.name;item.owner=ident.owner||''}}catch(e){}if(trackRadarAlgorithmic(item.name))continue;placements.push(item)}return{track,artist,placements,diagnostics:{queries:diagnostics.queries,rawCount:diagnostics.rawCount,spotifyCandidates:diagnostics.spotifyCandidates.slice(0,30),rawSample:diagnostics.rawSample},notice:'Risultati basati su evidenze web pubbliche indicizzate; non sono una lista completa di Spotify.'}}
export default {async fetch(request,env){const url=new URL(request.url);if(url.pathname==='/api/track-radar/import-match'&&request.method==='POST'){try{return json(await s4aImport(await request.json(),env))}catch(e){return json({error:e.message||'Errore import placement'},500)}}if(url.pathname==='/api/artist-radar/contact'&&request.method==='POST'){try{return json(await artistRadarContactLookup(await request.json(),env))}catch(e){return json({error:e.message||'Errore contatti Artist Radar'},500)}}if(url.pathname==='/api/artist-radar'&&request.method==='POST'){try{return json(await artistRadarScan(await request.json(),env))}catch(e){return json({error:e.message||'Errore Artist Radar'},500)}}if(url.pathname==='/api/track-radar'&&request.method==='POST'){try{return json(await trackRadarScan(await request.json(),env))}catch(e){return json({error:e.message||'Errore Track Radar'},500)}}if(url.pathname==='/api/spotify-track'&&request.method==='GET'){try{const raw=url.searchParams.get('url')||'';const u=new URL(raw);const parts=u.pathname.split('/').filter(Boolean);if(u.hostname!=='open.spotify.com'||parts[0]!=='track'||!parts[1])return json({error:'Link Spotify non valido'},400);const clean='https://open.spotify.com/track/'+parts[1];const r=await fetch('https://open.spotify.com/oembed?url='+encodeURIComponent(clean));if(!r.ok)return json({error:'Brano Spotify non trovato'},400);const token=await spotifyAccessToken(env);if(!token)return json({error:'Spotify non disponibile'},503);const tr=await fetch('https://api.spotify.com/v1/tracks/'+encodeURIComponent(parts[1]),{headers:{Authorization:'Bearer '+token}});if(!tr.ok)return json({error:'Brano Spotify non trovato'},400);const td=await tr.json();const names=(td.artists||[]).map(a=>a.name).filter(Boolean);let genres=[];const aid=td.artists&&td.artists[0]&&td.artists[0].id;if(aid){try{const ar=await fetch('https://api.spotify.com/v1/artists/'+encodeURIComponent(aid),{headers:{Authorization:'Bearer '+token}});if(ar.ok){const ad=await ar.json();genres=(ad.genres||[]).filter(Boolean).slice(0,4)}}catch(e){}}let genreSource=genres.length?'Spotify':'',genreConfidence=genres.length?'MEDIA':'';if(!genres.length&&names.length){try{const qs=[names[0]+' '+String(td.name||'')+' music genre style',names[0]+' '+String(td.name||'')+' melodic techno melodic house progressive house techno house',names[0]+' music genre'];const catalog=['melodic techno','melodic house','progressive house','tech house','deep house','afro house','organic house','minimal techno','hard techno','techno','trance','house','electronic','dance pop','pop','hip hop','rap','indie'];const scores=new Map();for(const q of qs){const sr=await smartSearch(q,env,8);for(const row of sr.results||[]){const text=String((row.title||'')+' '+(row.description||'')).toLowerCase();const specificHits=catalog.filter(g=>g.includes(' ')&&text.includes(g));for(const g of catalog){if((g==='house'||g==='techno')&&specificHits.some(x=>x.endsWith(g)))continue;let pos=0,start=0;const weight=g.includes(' ')?3:1;while((pos=text.indexOf(g,start))>=0){scores.set(g,(scores.get(g)||0)+weight);start=pos+g.length}}}}const ranked=[...scores.entries()].sort((a,b)=>b[1]-a[1]||b[0].length-a[0].length);if(ranked.length){const best=ranked[0][0],bestScore=ranked[0][1];genres=[best];for(const x of ranked.slice(1)){if(x[1]>=Math.max(2,bestScore-1)&&!best.includes(x[0])&&!x[0].includes(best))genres.push(x[0]);if(genres.length>=3)break}genreSource='Web';genreConfidence=bestScore>=4?'ALTA':bestScore>=2?'MEDIA':'BASSA'}}catch(e){}}return json({track:String(td.name||'').trim(),artist:names.join(', '),image:String((td.album&&td.album.images&&td.album.images[0]&&td.album.images[0].url)||d.thumbnail_url||''),spotifyUrl:clean,trackId:parts[1],genres:genres.slice(0,3),genreSource,genreConfidence})}catch(e){return json({error:'Link Spotify non valido'},400)}}
