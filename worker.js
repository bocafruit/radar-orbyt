const VERSION = 'RADAR v0.4.7.62 Cloud';
...
function artistRadarEmbedEntity(html){
  ...
  if(!m)return null;
  try{let node=JSON.parse(m[1]);for(const k of ['props','pageProps','state','data','entity']){if(!node||typeof node!=='object')return null;node=node[k]}return node&&typeof node==='object'?node:null}catch(e){return null}
}
async function artistRadarDirectPlaylistCheck(x,cat){
  ...
  try{
    ...
    const entity=artistRadarEmbedEntity(await r.text());if(!entity)return {available:false,reason:'embed-parse',matches:[]};
    const list=Array.isArray(entity.trackList)?entity.trackList:[],total=Number(entity.trackCount||entity.totalTracks||0)||0,byId=new Map(),byName=new Map();
    ...
    const matches=[];for(const t of list){const uri=String(t?.uri||''),id=uri.startsWith('spotify:track:')?uri.split(':').pop():String(t?.id||''),title=String(t?.title||t?.name||'').trim(),subtitle=String(t?.subtitle||t?.artist||'').trim();let hit=id&&byId.get(id);if(!hit&&title&&normalize(subtitle).includes(normalize(cat.artist)))hit=byName.get(normalize(title));if(hit&&!matches.includes(hit))matches.push(hit)}
  ...
  }catch(e){return {available:false,reason:e&&e.name==='AbortError'?'embed-timeout':'embed-error',matches:[]}}finally{clearTimeout(timer)}
}
async function artistRadarVerifyCandidate(x,cat,env){
  ...
  if(direct.available&&direct.complete)return {...x,verification:'REJECTED',verificationLabel:'SCARTATO · NON PRESENTE',verificationScore:0,placementConfidence:0,placementBaseScore:0,placementVerificationBoost:0,placementReason:'Tracklist pubblica Spotify completa controllata: nessuna traccia del catalogo artista trovata.',directVerified:false,directSource:direct.source,directMatchedTracks:[],directVisibleTracks:direct.visibleTracks,directTotalTracks:direct.totalTracks,directPartial:false,verificationDomains:['open.spotify.com'],verificationEvidence:['spotify-embed complete no-match']};
