const VERSION = 'RADAR v0.4.7.42 Cloud';
...
async function trackRadarScan(input,env){const track=String(input?.track||'').trim(),artist=String(input?.artist||'').trim();if(!track||!artist)throw new Error('Metadati brano mancanti');const queries=['open.spotify.com/playlist \"'+track+'\" \"'+artist+'\"','Spotify playlist \"'+track+'\" \"'+artist+'\"','open.spotify.com \"'+track+'\" \"'+artist+'\"','\"'+track+'\" \"'+artist+'\" playlist Spotify','\"'+track+'\" \"'+artist+'\" open.spotify.com/playlist'];const batches=await Promise.all(queries.map(q=>smartSearch(q,env,30)));const rawRows=batches.flatMap(x=>x.results||[]);const diagnostics={queries:queries.map((q,i)=>({query:q,provider:batches[i]?.provider||'',count:(batches[i]?.results||[]).length})),rawCount:rawRows.length,spotifyCandidates:[]};const map=new Map();for(const r of rawRows){const text=normalize((r.title||'')+' '+(r.description||'')+' '+(r.url||''));let clean=cleanPlaylistUrl(r.url||'');if(!clean){const blob=String((r.title||'')+' '+(r.description||''));const m=blob.match(/https?:\/\/open\.spotify\.com\/playlist\/[A-Za-z0-9]+/i);if(m)clean=cleanPlaylistUrl(m[0])}if(!clean)continue;const trackHit=text.includes(normalize(track)),artistHit=text.includes(normalize(artist));diagnostics.spotifyCandidates.push({url:clean,title:String(r.title||'').slice(0,120),trackHit,artistHit});if(!trackHit||!artistHit)continue;const parts=new URL(clean).pathname.split('/').filter(Boolean);if(parts[0]!=='playlist'||!parts[1])continue;const id=parts[1],evidence=String((r.title||'')+' — '+(r.description||'')).trim();const titleText=normalize(r.title||''),descText=normalize(r.description||'');const explicit=descText.includes(normalize(track))&&descText.includes(normalize(artist));const confidence=explicit?'confirmed':'probable';if(!map.has(id)||evidence.length>map.get(id).evidence.length)map.set(id,{playlistId:id,spotifyUrl:clean,name:String(r.title||'Spotify playlist').trim(),confidence,evidence:evidence.slice(0,280)});}const placements=[];for(const item of [...map.values()].slice(0,20)){try{const ident=await spotifyPlaylistIdentity(item.spotifyUrl,env);if(ident){item.name=ident.name||item.name;item.owner=ident.owner||''}}catch(e){}placements.push(item)}return{track,artist,placements,diagnostics:{queries:diagnostics.queries,rawCount:diagnostics.rawCount,spotifyCandidates:diagnostics.spotifyCandidates.slice(0,30)},notice:'Risultati basati su evidenze web pubbliche indicizzate; non sono una lista completa di Spotify.'}}
